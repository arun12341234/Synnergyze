import React from 'react'
import './Regist.css'
import { Link } from 'react-router-dom'
const Registaion = () => {
    return (
        <nav id='filter'>
            <main className='regit'>
                <h1>*  Registaion Form  *</h1>
                <div className='sec-to'>
                <i class="ri-user-fill"></i>
                    <input type="text" placeholder='Enter Your Name' id='name'  required/>
                </div>
                <div className='sec-to'>
                <i class="ri-mail-fill"></i>
                    <input type="email" placeholder='Enter Your Email Address' id='email' required/>
                </div>
                <div className='sec-to'>
                <i class="ri-lock-fill"></i>
                    <input type="password" placeholder='Enter Your Password' id='password' required/>
                </div>
               <div className='radio-part'>
                <input type="radio"  id='male'   value="male" name='fav-ge' /><label htmlFor="male">Male</label>
                <input type="radio" id='female'  value="female" name='fav-ge'/><label htmlFor="female">Female</label>
               </div>
                <button className='btnn-2'>Submit</button>
                <div className='last-page'>
                    <p>Have Already an account?<Link to="/login">Login here</Link></p>
                </div>
                <div className='lower-power' >
                    <div className='lower-2'></div>
                    <h3>Or</h3>
                    <div className='lower-2'></div>
                </div>
                <div className='login-fg'>
                    <button className='fb'><img src="data:image/png;base64," alt=""width="30px" height="25px" />   Login with Facebook</button>
                    <button>  <img src="https://www.outsystems.com/forge/DownloadResource.aspx?FileName=&ImageBinaryId=43951" alt="" width="30px" height="25px"/>  Login with Google</button>
                </div>
            </main>
        </nav>
    )
}

export default Registaion
